...
export default function NewsMenu() {
    ...
    if (data.count === 0) return <p className="text-center">Loading...</p>;
    ...
    return (
        <div className="flex flex-col gap-4 mb-5">
            ...
            {mainNews && (
                <Link
                    to={`/n/${mainNews.id}`}
                    className="grid grid-cols-1 md:grid-cols-2 py-4"
                >
                    <div className="pr-3">
                        <div className="flex justify-between">
                            <p className="font-thin text-gray-500">
                                {mainNews.User?.username ??
                                    mainNews.User?.email}
                            </p>
                            <p className="font-thin text-gray-500">
                ...
                {restNews.map((newsItem, i) => (
                    <div key={i}>
                        {newsItem && (
                            <ArticleCard
                                newsItem={newsItem}
                                formatRelativeTime={formatCurrentDate}
                                Link={Link}
                            />
                        )}
                    </div>
